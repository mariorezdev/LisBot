...
public record OneForAllListener(CommandManager commandManager) implements Listener {
    ...
    public void onNewMessage(Whatsapp whatsapp, MessageInfo messageInfo) {
        ...
        var optionalCommand = commandManager.findCommand(textMessage.text());

        optionalCommand.ifPresent(command -> command.onCommand(whatsapp, messageInfo));
    }
}
